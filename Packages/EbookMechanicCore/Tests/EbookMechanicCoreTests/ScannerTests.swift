...
final class ScannerTests: XCTestCase {
  func testScanForCorruptionAndEmptyFolders() async throws {
    ...
    try FileManager.default.createDirectory(
      at: goodEPUB.deletingLastPathComponent(), withIntermediateDirectories: true, attributes: nil)
    try FileManager.default.createDirectory(
      at: badEPUB.deletingLastPathComponent(), withIntermediateDirectories: true, attributes: nil)
    try FileManager.default.createDirectory(
      at: emptyFolder, withIntermediateDirectories: true, attributes: nil)

    try TestFixtures.createValidEPUB(at: goodEPUB)
    try TestFixtures.createEPUBMissingManifest(at: badEPUB)
    ...
    XCTAssertTrue(
      FileManager.default.fileExists(atPath: corruptedDir.appendingPathComponent("bad.epub").path))
  ...
  func testRepairCorruptedFiles() async throws {
    ...
    try FileManager.default.createDirectory(
      at: badPDF.deletingLastPathComponent(), withIntermediateDirectories: true, attributes: nil)
    ...
    let validation = await validator.validate(url: badPDF, as: .pdf)
  ...
  func testNormalizeEPUBRepairsManifestAndNCX() async throws {
    ...
    try FileManager.default.createDirectory(
      at: epubURL.deletingLastPathComponent(), withIntermediateDirectories: true, attributes: nil)
    ...
    XCTAssertTrue(
      opfString.contains("<spine") && opfString.contains("toc="),
      "Spine should reference NCX toc id")
