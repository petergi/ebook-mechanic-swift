...
final class HTMLReportGeneratorTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        let corruptedFile1 = CorruptedFile(url: mockRootDirectory.appendingPathComponent("book with <tags>.epub"), reason: "Missing mimetype <script>alert(1)</script>", size: 1024, status: .nonCompliant)
        let corruptedFile2 = CorruptedFile(url: mockRootDirectory.appendingPathComponent("path/to/book&name.pdf"), reason: "Invalid EOF & special chars", size: 2048, status: .corrupt)
        ...
        mockRepairs = [
            ...
            RepairResult(success: false, message: "Could not fix PDF & Co.", fixed: false, fileURL: corruptedFile2.url)
        ]
    }

    func testHTMLStructureAndContent() throws {
        let htmlString = try reportGenerator.generate(from: mockScanResult, rootDirectory: mockRootDirectory, corruptedDirectoryName: mockCorruptedDirectoryName, repairs: mockRepairs)
        ...
        XCTAssertTrue(htmlString.contains("<h2>Corruption Scan Summary</h2>"))
        XCTAssertTrue(htmlString.contains("<h3>By File Type</h3>"))
        XCTAssertTrue(htmlString.contains("<h2>Empty Folders Summary</h2>"))
        XCTAssertTrue(htmlString.contains("<h2>Corrupted Files Details</h2>"))
        XCTAssertTrue(htmlString.contains("<h2>Repair Attempts</h2>"))
    }
    ...
    func testStatusBadgesAndCollapsibleDetails() throws {
        ...
        XCTAssertTrue(htmlString.contains("<span class=\"badge nonCompliant\">⚠️ Non-Compliant</span>"))
