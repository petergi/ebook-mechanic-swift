...
public struct EPUBMetadataRepairer: @unchecked Sendable {
  ...
  public func repair(at url: URL) throws -> RepairResult {
    ...
    do {
      ...
      guard let containerEntry = entry(named: "META-INF/container.xml", in: archive.entries) else {
        return RepairResult(
          success: false, message: "Missing container.xml", fixed: false, fileURL: url)
      }

      guard let opfPath = extractOPFPath(from: containerEntry.data) else {
        return RepairResult(
          success: false, message: "Unable to locate OPF path", fixed: false, fileURL: url)
      }

      guard
        let opfIndex = archive.entries.firstIndex(where: {
          $0.name.caseInsensitiveCompare(opfPath) == .orderedSame
        })
      else {
        return RepairResult(
          success: false, message: "Missing OPF file at \(opfPath)", fixed: false, fileURL: url)
      }

      let opfData = archive.entries[opfIndex].data
      let fallbackTitle = url.deletingPathExtension().lastPathComponent
      let (updatedData, changed) = try repairOPFMetadata(
        opfData: opfData, fallbackTitle: fallbackTitle)

      guard changed else {
        return RepairResult(
          success: true, message: "No metadata updates needed", fixed: false, fileURL: url)
      }
      ...
      return RepairResult(
        success: true, message: "EPUB metadata repaired", fixed: true, fileURL: url)
    } catch {
      try? restoreBackup(from: backupURL, to: url)
      return RepairResult(
        success: false, message: "Failed to repair metadata: \(error.localizedDescription)",
  ...
  private func ensureDCElement(
    named localName: String, value: String, in metadata: XMLElement, dcNamespace: String
  ) -> Bool {
  ...
  private func normalizeDateElements(in metadata: XMLElement, dcNamespace: String) -> Bool {
    ...
    for element in metadata.elements(forLocalName: "date", uri: dcNamespace) {
      guard let value = element.stringValue?.trimmingCharacters(in: .whitespacesAndNewlines),
        !value.isEmpty
      else {
