...
struct OPFValidator {
  ...
  func validate(archive: ZipArchive, containerData: Data) -> OPFValidationResult {
      ...
      if let foundEntry = archive.entries.first(where: {
        ...
        issues.append(
          ValidationIssue(
            severity: .warning,
            message:
              "OPF file path case mismatch: container references '\(opfPath)' but found '\(foundEntry.name)'"
          ))
        return validateOPFContent(
          opfData: foundEntry.data, opfPath: foundEntry.name, archive: archive, issues: &issues)
      }
    ...
    return validateOPFContent(
      opfData: opfEntry.data, opfPath: opfPath, archive: archive, issues: &issues)
  }

  private func validateOPFContent(
    opfData: Data, opfPath: String, archive: ZipArchive, issues: inout [ValidationIssue]
  ) -> OPFValidationResult {
    // Parse OPF as XML
    ...
    // Validate manifest
    let manifestItems = validateManifest(
      packageElement: packageElement, opfPath: opfPath, archive: archive, issues: &issues)
  ...
  private func validateManifest(
    packageElement: XMLElement, opfPath: String, archive: ZipArchive,
      ...
      if !archive.entries.contains(where: { $0.name == absolutePath }) {
        issues.append(
          ValidationIssue(
            severity: .error,
            message:
              "Manifest item '\(id)' references missing file: \(href) (resolved to \(absolutePath))"
          ))
      }
  ...
  private func validateSpine(
    packageElement: XMLElement, manifestItems: Set<String>, issues: inout [ValidationIssue]
  ) {
