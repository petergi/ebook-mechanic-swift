...
public actor FileScanner {
  ...
  private struct ValidationOutcome {
    ...
    let fileType: EbookFileType
    let validationTime: TimeInterval
    let usedExternalTool: Bool
    let cacheHit: Bool
  }
  ...
  public func scanForCorruption(progress: ProgressHandler? = nil) async throws -> ScanResult {
    ...
    var result = ScanResult(
      ...
      corruptedFiles: [],
      breakdowns: Dictionary(
        uniqueKeysWithValues: EbookFileType.allCases.map { ($0, FormatBreakdown()) })
    )
    ...
    try await withThrowingTaskGroup(of: ValidationOutcome.self) { group in
        ...
        if let indexToSubmit = filesToProcess.firstIndex(where: { !$0.0 }) {
          ...
          group.addTask {
            ...
            return ValidationOutcome(
              ...
              usedExternalTool: usedExternalTool,
              cacheHit: false)
          }
        }
      }
      ...
      do {
        for try await outcome in group {
          ...
          if outcome.usedExternalTool {
            externalToolCalls += 1
          }
          validationTimes[outcome.fileType, default: []].append(outcome.validationTime)
          ...
          progress?(
            ProgressEvent(
              ...
              total: selectedFiles.count,
              currentItem: outcome.validation.url.lastPathComponent,
              concurrentValidationCount: concurrentCount
            ))
    ...
    self.performanceMetrics = PerformanceMetrics(
      ...
      parallelEfficiencyRatio: totalFiles > 0
        ? (totalValidationTime / totalFiles)
          / (totalValidationTime / (totalFiles * Double(maxConcurrentValidations))) : 0,
  ...
  public func moveCorruptedFiles(progress: ProgressHandler? = nil) async throws {
    ...
    try fileManager.createDirectory(
      at: corruptedDirURL, withIntermediateDirectories: true, attributes: nil)
    ...
    for (index, url) in epubs.enumerated() {
      ...
      do {
        ...
        guard let first = archive.entries.first else {
          ...
          progress?(
            ProgressEvent(
              stage: .normalizingFiles, completed: index + 1, total: epubs.count,
              currentItem: url.lastPathComponent))
          continue
        }
        ...
        let containerEntry = archive.entries.first(where: {
        ...
        let alreadyNormalized =
          hasMimetypeFirst && mimetypeUncompressed && mimetypeCorrect && hasContainer
          && allNonMimetypeDeflate && !hasExtraneousFiles && containerCasingCorrect
        ...
        let normalization = archiveNormalizer.normalize(
          entries: filteredEntries, containerData: containerData)
  ...
  public func repairCorruptedFiles(progress: ProgressHandler? = nil) async -> (
    results: [RepairResult], repairedCount: Int
  ) {
  ...
  public func generateReport(into directory: URL) async throws -> [URL] {
    ...
    for format in reportFormats {
      ...
      let reportContent = try generator.generate(
        from: lastResult, rootDirectory: rootDirectory,
        corruptedDirectoryName: corruptedDirectoryName, repairs: lastRepairResults)
