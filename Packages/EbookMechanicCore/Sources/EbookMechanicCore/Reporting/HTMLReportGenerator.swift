...
struct HTMLReportGenerator {
  func generate(
    ...
    let summaryRows = renderSummaryRows(result: result)
    let corruptedSection =
      result.corruptedFiles.isEmpty
      ? "<h2 class='section-header status-ok'>✅ No Corrupted Files Found</h2>"
      : generateCorruptedFilesDetails(result: result, rootDirectory: rootDirectory)
    let emptyFoldersSection: String
    if result.emptyFolders.isEmpty {
      if result.totalFolders > 0 {
        emptyFoldersSection = "<h2 class='section-header status-ok'>✅ No Empty Folders Found</h2>"
      } else {
        emptyFoldersSection = ""
      }
    } else {
      emptyFoldersSection = generateEmptyFoldersDetails(result: result, rootDirectory: rootDirectory)
    }

    let repairsSection: String
    if repairs.isEmpty {
      repairsSection = ""
    } else {
      repairsSection = generateRepairAttempts(repairs: repairs, rootDirectory: rootDirectory)
    }

    let html = """
      <!DOCTYPE html>
      <html lang="en">
      \(renderHead(timestamp: timestamp))
      \(renderBody(
        timestamp: timestamp,
        ...
        corruptedSection: corruptedSection,
        emptyFoldersSection: emptyFoldersSection,
        repairsSection: repairsSection
      ))
      </html>
      """
    return html
  ...
  private func renderHead(timestamp: String) -> String {
    """
    <head>
        <meta charset="UTF-8">
  ...
  private func renderBody(
    ...
    emptyFoldersSection: String,
    repairsSection: String
  ) -> String {
    """
    <body>
        <div class="container">
  ...
  private func renderSummaryRows(result: ScanResult) -> String {
    var rows = ""
    for type in EbookFileType.allCases {
      let breakdown = result.breakdown(for: type)
      let statusEmoji = breakdown.corrupted > 0 ? "❌" : "✅"
      let statusClass = breakdown.corrupted > 0 ? "corrupt" : "ok"
      let statusLabel = breakdown.corrupted > 0 ? "Corrupted" : "OK"
      rows += """
        <tr>
            <td>\(type.fileExtension.uppercased())</td>
            <td>\(breakdown.corrupted)</td>
            <td>\(breakdown.total)</td>
            <td><span class="badge \(statusClass)">\(statusEmoji) \(statusLabel)</span></td>
        </tr>
        """
    }
    return rows
    ...
    for type in EbookFileType.allCases {
      ...
      for file in files {
        ...
        if let pdfDetails = file.pdfValidationDetails {
          let structureClass = pdfDetails.structureValid ? "ok" : "corrupt"
          let structureLabel = pdfDetails.structureValid ? "Valid" : "Invalid"
          let xrefClass = pdfDetails.xrefValid ? "ok" : "corrupt"
          let xrefLabel = pdfDetails.xrefValid ? "Valid" : "Invalid"
          let pageTreeClass = pdfDetails.pageTreeValid ? "ok" : "corrupt"
          let pageTreeLabel = pdfDetails.pageTreeValid ? "Valid" : "Invalid"
          let streamErrors = pdfDetails.streamErrors.isEmpty
            ? "None"
            : "<span class=\"badge corrupt\">\(pdfDetails.streamErrors.joined(separator: ", ").htmlEscaped())</span>"
          let encryption = pdfDetails.encryptionInfo?.htmlEscaped() ?? "None"
          let conforms = pdfDetails.conformsToStandard?.htmlEscaped() ?? "N/A"
          pdfDetailsHtml = """
            <div class="pdf-validation-details">
                <p><strong>PDF Structure Validation:</strong></p>
                <ul>
                    <li>Structure: <span class="badge \(structureClass)">\(structureLabel)</span></li>
                    <li>Cross-Reference Table: <span class="badge \(xrefClass)">\(xrefLabel)</span></li>
                    <li>Page Tree: <span class="badge \(pageTreeClass)">\(pageTreeLabel)</span></li>
                    <li>Stream Errors: \(streamErrors)</li>
                    <li>Encryption: \(encryption)</li>
                    <li>Conforms To: \(conforms)</li>
                </ul>
            </div>
            """
        }
