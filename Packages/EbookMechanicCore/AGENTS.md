...
- `make -f Makefile.swift check-format`: lint formatting with swift-format.
- `make lint`: run SwiftLint.
- `make -f Makefile.swift ci`: build-all + test-all + check-format.

## Coding Style & Naming Conventions
- Swift formatting is enforced via `swift-format` (`make format` / `make -f Makefile.swift check-format`).
