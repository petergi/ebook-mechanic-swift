# ADR 002: EbookMechanicEPUBCLI Feature Enhancements

## Status
Proposed

## Context
The `EbookMechanicEPUBCLI` has been enhanced with new features to provide more comprehensive EPUB validation and repair capabilities. These enhancements include new command-line flags for detailed specification checking, warning display, accessibility compliance, and a metadata repair function. This ADR documents the decisions behind these feature additions.

## Decision
The following features have been added to `EbookMechanicEPUBCLI`:
...
*   `Packages/EbookMechanicEPUBCLI/Sources/EbookMechanicEPUBCLI/main.swift`: Modified to include the new flags and logic for handling them.
*   `Packages/EbookMechanicEPUBCLI/Sources/EbookMechanicEPUBCLI/EPUBReportFormatter.swift`: Modified to enhance reporting capabilities.
*   `Packages/EbookMechanicCore/Sources/EbookMechanicCore/Validation/ExternalValidators.swift`: Updated to accept `showWarnings` and `accessibility` parameters for `epubcheck` execution.
*   `Packages/EbookMechanicCore/Sources/EbookMechanicCore/Support/ZipArchive.swift`: Used for reading and writing EPUB files during metadata repair.
