...
struct EbookMechanicCLI: ParsableCommand {
    ...
    @Option(name: .long, help: "The format of the report to generate.", completion: .list(["markdown", "json", "csv", "html"]))
    var reportFormat: String = "markdown"

    @Flag(name: .long, help: "Normalize EPUB files.")
    var normalizeEpubs: Bool = false
    
    @Flag(name: .long, help: "Force normalization of EPUB files, even if they appear to be normalized.")
    ...
    func run() throws {
        ...
        Task {
            do {
                ...
                let validator = FileValidator(useExternalEPUBValidator: useEpubcheck, useExternalPDFValidator: false, cacheSize: noCache ? 0 : 1000)
                ...
                if normalizeEpubs {
                    ...
                    let (normalized, skipped) = await scanner.normalizeEPUBs(force: forceNormalize, dryRun: dryRun, progress: printer.handle)
                ...
                if report {
                    let generatedReportURLs = try await scanner.generateReport(into: URL(fileURLWithPath: dir))
