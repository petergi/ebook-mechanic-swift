...
final class CLIConfigurationTests: XCTestCase {
    ...
    func testDirectoryWithSpaces() throws {
        let config = try CLIConfiguration.parse(arguments: ["ebook-mechanic", "-d", "/Users/test/My Books"])
    ...
    func testFlagsInDifferentOrders() throws {
        let config1 = try CLIConfiguration.parse(arguments: ["ebook-mechanic", "-r", "-d", "/tmp", "--quiet"])
        let config2 = try CLIConfiguration.parse(arguments: ["ebook-mechanic", "--quiet", "-d", "/tmp", "-r"])
        let config3 = try CLIConfiguration.parse(arguments: ["ebook-mechanic", "-d", "/tmp", "--quiet", "-r"])
    ...
    func testHomeDirectoryPath() throws {
        let config = try CLIConfiguration.parse(arguments: ["ebook-mechanic", "-d", "~/Documents/Books"])
