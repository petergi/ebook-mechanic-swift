...
extension PDFMechanicCLI {
    struct Validate: ParsableCommand {
        ...
        @Option(name: .customLong("extract-metadata"), help: "Extract metadata from the PDF to json or yaml.", completion: .list(["json", "yaml"]))
        ...
        private func extractPDFMetadata(at url: URL, format: String) async throws {
            ...
            let outputURL = url.deletingPathExtension().appendingPathExtension("metadata.\(outputExtension)")
                    ...
                    if optimize {
                        for file in result.okFiles.filter({ let ext = $0.url.pathExtension.lowercased(); return ext == "pdf" || ext == "azw4" }) {
                            if !isDryRun {
                                ...
                                let (success, output) = await ExternalValidators.optimizePDF(at: file.url.path, outputPath: optimizedURL.path)
                                if success {
                                    print("✅ Optimized \(file.url.lastPathComponent) → \(optimizedURL.lastPathComponent)")
                                } else {
                                    print("❌ Failed to optimize \(file.url.lastPathComponent): \(output)")
                                }
                            } else {
                                let optimizedURL = file.url.deletingPathExtension().appendingPathExtension("optimized.pdf")
                                print("DRY RUN: Would optimize \(file.url.lastPathComponent) → \(optimizedURL.lastPathComponent)")
                            }
                        }
                    }
