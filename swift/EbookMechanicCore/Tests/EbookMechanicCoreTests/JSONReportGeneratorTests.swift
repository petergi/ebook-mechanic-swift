...
final class JSONReportGeneratorTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        let corruptedFile1 = CorruptedFile(url: mockRootDirectory.appendingPathComponent("book1.epub"), reason: "Missing mimetype", size: 1024, status: .nonCompliant)
        let corruptedFile2 = CorruptedFile(url: mockRootDirectory.appendingPathComponent("path/to/book,2.pdf"), reason: "Invalid EOF marker", size: 2048, status: .corrupt)
        ...
        mockRepairs = [
            ...
            RepairResult(success: false, message: "Could not fix PDF", fixed: false, fileURL: corruptedFile2.url)
        ]
    }

    func testJSONStructureAndContent() throws {
        let jsonString = try reportGenerator.generate(from: mockScanResult, rootDirectory: mockRootDirectory, corruptedDirectoryName: mockCorruptedDirectoryName, repairs: mockRepairs)
    ...
    func testEmptyScanResult() throws {
        ...
        let jsonString = try reportGenerator.generate(from: emptyResult, rootDirectory: mockRootDirectory, corruptedDirectoryName: mockCorruptedDirectoryName, repairs: [])
    ...
    func testSpecialCharactersInPathsAndReasons() throws {
        let specialCharURL = mockRootDirectory.appendingPathComponent("book with spaces & other-chars/book,name's.pdf")
        let specialCharReason = "Reason with, commas\"quotes\"and\nnewlines."
        
        let corruptedFile = CorruptedFile(url: specialCharURL, reason: specialCharReason, size: 100, status: .corrupt)
        let specialResult = ScanResult(totalFiles: 1, corruptedFiles: [corruptedFile])
        
        let jsonString = try reportGenerator.generate(from: specialResult, rootDirectory: mockRootDirectory, corruptedDirectoryName: mockCorruptedDirectoryName, repairs: [])
