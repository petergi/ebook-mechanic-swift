...
final class ExternalPDFValidatorTests: XCTestCase {
    ...
    private func createMockPdfcpu(exitCode: Int, stdout: String = "", stderr: String = "", delay: TimeInterval = 0) throws {
        let scriptContent = """
        #!/bin/bash
        sleep \(delay)
        echo "\(stdout)"
        ...
        try FileManager.default.setAttributes([.posixPermissions: 0o755], ofItemAtPath: mockPdfcpuPath.path)
    }
    ...
    func testIsPdfcpuInstalled() async throws {
        ...
        XCTAssertTrue(await ExternalPDFValidator.isPdfcpuInstalled())
    }
    ...
    func testStructureErrorDetection() async throws {
        ...
        XCTAssertTrue(result.pdfValidationDetails!.streamErrors.contains(where: { $0.contains("EOF marker missing") }))
        XCTAssertEqual(result.validationLevel, .comprehensive)
    }
    ...
    func testPdfcpuNotInstalledFallback() async throws {
        ...
        let result = await validator.validate(fileURL: dummyFileURL, pdfcpuPath: "/nonexistent/path/pdfcpu")
