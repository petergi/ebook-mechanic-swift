...
final class CSVReportGeneratorTests: XCTestCase {
  ...
  override func setUpWithError() throws {
    ...
    let corruptedFile1 = CorruptedFile(
      url: mockRootDirectory.appendingPathComponent("book with,comma.epub"),
      reason: "Missing mimetype", size: 1024, status: .nonCompliant)
    let corruptedFile2 = CorruptedFile(
      url: mockRootDirectory.appendingPathComponent("path/to/book\"quotes\".pdf"),
      reason: "Invalid EOF, marker\nwith newline", size: 2048, status: .corrupt)
  ...
  func testCSVHeadersAndContent() throws {
    let csvString = try reportGenerator.generate(
    ...
    // Test data rows
    XCTAssertTrue(csvString.contains("\"book with,comma.epub\""))
    XCTAssertTrue(csvString.contains("\"nonCompliant\""))
    XCTAssertTrue(csvString.contains("\"epub\""))
    XCTAssertTrue(csvString.contains("\"Missing mimetype\""))
    XCTAssertTrue(csvString.contains(",\"1024\","))

    XCTAssertTrue(csvString.contains("\"path/to/book\"\"quotes\"\".pdf\""))
    XCTAssertTrue(csvString.contains("\"corrupt\""))
    XCTAssertTrue(csvString.contains("\"pdf\""))
    XCTAssertTrue(csvString.contains("\"Invalid EOF, marker\nwith newline\""))
    XCTAssertTrue(csvString.contains(",\"2048\","))
  }
  ...
  func testCSVEscaping() throws {
    ...
    let corruptedFile = CorruptedFile(
      url: mockRootDirectory.appendingPathComponent("file,name.epub"), reason: specialCharReason,
      size: 123, status: .corrupt)
    let customScanResult = ScanResult(totalFiles: 1, corruptedFiles: [corruptedFile])

    let csvString = try reportGenerator.generate(
      from: customScanResult, rootDirectory: mockRootDirectory,
  ...
  func testRepairResultsSection() throws {
    ...
    XCTAssertTrue(csvString.contains("\"book with,comma.epub\",true,true,\"Fixed mimetype\"\n"))
    XCTAssertTrue(
      csvString.contains(
        "\"path/to/book\"\"quotes\"\".pdf\",false,false,\"Could not fix PDF: reason, with \"\"quotes\"\"\"\n"
      ))
  }
}
