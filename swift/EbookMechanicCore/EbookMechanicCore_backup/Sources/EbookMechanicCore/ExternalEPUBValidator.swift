...
public struct ExternalEPUBValidator {
  ...
  public func validate(
    fileURL: URL, epubcheckPath: String? = nil, timeout: TimeInterval = 60,
    showWarnings: Bool = false, accessibility: Bool = false
  ) async -> ValidationResult {
    guard await Self.isEpubcheckInstalled() else {
      return ValidationResult(
        url: fileURL, size: 0, isValid: false, reason: "Epubcheck not installed.",
    ...
    do {
      ...
      return ValidationResult(
        url: fileURL, size: 0, isValid: isValid, reason: reason, status: status,
      ...
      return ValidationResult(
        url: fileURL, size: 0, isValid: false, reason: reason, status: .validationError,
        validationLevel: .comprehensive)
    } catch {
      return ValidationResult(
        url: fileURL, size: 0, isValid: false,
