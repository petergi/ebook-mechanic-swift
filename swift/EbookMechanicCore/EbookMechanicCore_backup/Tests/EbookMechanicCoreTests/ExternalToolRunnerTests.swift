...
final class ExternalToolRunnerTests: XCTestCase {
  ...
  func testIsCommandAvailable() async throws {
    ...
    XCTAssertTrue(
      await ExternalToolRunner.isCommandAvailable(toolName), "Command should be available")
    XCTAssertFalse(
      await ExternalToolRunner.isCommandAvailable("definitely_not_available_\(UUID().uuidString)"),
