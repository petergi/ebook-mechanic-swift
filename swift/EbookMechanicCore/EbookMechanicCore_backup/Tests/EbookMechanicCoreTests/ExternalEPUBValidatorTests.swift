...
final class ExternalEPUBValidatorTests: XCTestCase {
    ...
    private func createMockEpubcheck(exitCode: Int, stdout: String = "", stderr: String = "", delay: TimeInterval = 0) throws {
        let scriptContent = """
        #!/bin/bash
        sleep \(delay)
        echo "\(stdout)"
        ...
        try FileManager.default.setAttributes([.posixPermissions: 0o755], ofItemAtPath: mockEpubcheckPath.path)
    }
    ...
    func testValidEPUBDetection() async throws {
        ...
        let result = await validator.validate(fileURL: dummyFileURL, epubcheckPath: mockEpubcheckPath.path)
    ...
    func testEpubcheckNotInstalledFallback() async throws {
        ...
        let result = await validator.validate(fileURL: dummyFileURL, epubcheckPath: "/nonexistent/path/epubcheck")
