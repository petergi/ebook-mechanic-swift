...
struct EPUBArchiveNormalizer {
    ...
    func normalize(entries originalEntries: [ZipEntry], containerData: Data?) -> Result {
        ...
        var opfPath = determineOPFPath(from: initialContainer, entries: workingEntries) ?? "OEBPS/content.opf"
        opfPath = Self.normalizeArchivePath(opfPath)

        if let actual = workingEntries.first(where: { $0.name.caseInsensitiveCompare(opfPath) == .orderedSame }) {
            opfPath = actual.name
            ...
            workingEntries.append(ZipEntry(name: opfPath, data: builder.buildInitialDocument(), compressionMethod: 8))
        }

        if let index = workingEntries.firstIndex(where: { $0.name == opfPath }) {
            let opfData = workingEntries[index].data
            let normalization = OPFDocumentNormalizer.normalize(opfPath: opfPath, opfData: opfData, entries: workingEntries)
...
private extension EPUBArchiveNormalizer {
    static func normalizeArchivePath(_ name: String) -> String {
    ...
    static func normalizeHTMLIfNeeded(for entry: ZipEntry) -> Data {
        let lowercased = entry.name.lowercased()
        guard lowercased.hasSuffix(".html") || lowercased.hasSuffix(".xhtml") || lowercased.hasSuffix(".htm") else {
            return entry.data
        }
        ...
        let missingCriticalTag = requiredTags.contains { trimmed.range(of: $0, options: .caseInsensitive) == nil }
...
"""
    ...
    func determineOPFPath(from containerData: Data, entries: [ZipEntry]) -> String? {
    ...
    static func buildContainerXML(fullPath: String) -> Data {
...
private struct ContainerDocument {
    ...
    init?(data: Data) {
        ...
        let rootfiles = root.children?.compactMap { $0 as? XMLElement }.filter {
            $0.localName == "rootfiles" || $0.name == "rootfiles"
        } ?? []

        if let rootfilesElement = rootfiles.first {
            let rootfileElements = rootfilesElement.children?.compactMap { $0 as? XMLElement }.filter {
...
private struct OPFDocumentNormalizer {
    static func normalize(opfPath: String, opfData: Data, entries: [ZipEntry]) -> (entries: [ZipEntry], opfData: Data, opfPath: String) {
        ...
        let metadata = package.firstChild(named: "metadata") ?? {
            let node = XMLElement(name: "metadata")
            node.addNamespace(XMLNode.namespace(withName: "dc", stringValue: "http://purl.org/dc/elements/1.1/") as! XMLNode)
            package.insertChild(node, at: 0)
            return node
        }()
        let manifest = package.firstChild(named: "manifest") ?? {
            let node = XMLElement(name: "manifest")
            package.addChild(node)
            return node
        }()
        let spine = package.firstChild(named: "spine") ?? {
        ...
        if needsNCX(version: version) {
            if let tocID = ensureNCX(opfPath: opfPath, manifest: &manifestItems, htmlIDs: htmlIDs, metadata: metadata, entries: &workingEntries) {
...
private struct OPFDocumentBuilder {
    ...
    private static func makeBaseDocument() -> XMLDocument {
        ...
        package.addNamespace(XMLNode.namespace(withName: "dc", stringValue: "http://purl.org/dc/elements/1.1/") as! XMLNode)
...
private struct ManifestInventory {
    ...
    static func from(manifest: XMLElement, opfPath: String) -> ManifestInventory {
        ...
        let items = children.compactMap { element -> Item? in
            ...
            return Item(id: id, href: normalizedHref, mediaType: mediaType, properties: properties, element: element)
        }
    ...
    mutating func ensureEntries(for entries: [ZipEntry]) {
        ...
        for entryName in archiveNames {
            ...
            element.addAttribute(XMLNode.attribute(withName: "href", stringValue: normalizedHref) as! XMLNode)
            element.addAttribute(XMLNode.attribute(withName: "media-type", stringValue: mediaType) as! XMLNode)
            items.append(Item(id: identifier, href: normalizedHref, mediaType: mediaType, properties: nil, element: element))
        }
    }
    ...
    static func makeIdentifier(for entryName: String, used: inout Set<String>) -> String {
        ...
        var candidate = baseName.replacingOccurrences(of: "[^A-Za-z0-9]+", with: "-", options: .regularExpression)
    ...
    static func relativePath(from base: String, to target: String) -> String {
        ...
        var index = 0
        while index < baseComponents.count && index < targetComponents.count && baseComponents[index] == targetComponents[index] {
            index += 1
        }
...
private extension OPFDocumentNormalizer {
    static func ensureNCX(opfPath: String, manifest: inout ManifestInventory, htmlIDs: [String], metadata: XMLElement, entries: inout [ZipEntry]) -> String? {
        ...
        element.addAttribute(XMLNode.attribute(withName: "media-type", stringValue: "application/x-dtbncx+xml") as! XMLNode)
        manifest.items.append(ManifestInventory.Item(id: id, href: href, mediaType: "application/x-dtbncx+xml", properties: nil, element: element))

        return id
    }

    static func nextAvailableNCXPath(opfDirectory: String, entries: [ZipEntry]) -> String {
        var suffix = 0
...
private extension XMLElement {
    func firstChild(named name: String) -> XMLElement? {
        return children?.compactMap { $0 as? XMLElement }.first(where: { element in
            ...
            if let suffix = name.split(separator: ":").last, let local = element.localName, local == suffix {
                return true
            }
            return false
        })
    ...
    func addAttributeIfNeeded(name: String, value: String) {
    ...
    func replaceChildren(with nodes: [XMLNode]) {
        setChildren(nodes)
    }
}
