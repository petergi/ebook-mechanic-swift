...
struct CSVReportGenerator {
    func generate(from result: ScanResult, rootDirectory: URL, corruptedDirectoryName: String, repairs: [RepairResult]) throws -> String {
        var csv = "FilePath,Status,Format,Reason,FileSize,Fingerprint\n"

        // Data rows
        for file in result.corruptedFiles {
            let relativePath = file.url.path.replacingOccurrences(of: rootDirectory.path, with: "").trimmingCharacters(in: CharacterSet(charactersIn: "/"))
            ...
            csv += "\"\(escapeCSV(relativePath))\",\"\(escapeCSV(file.status.rawValue))\",\"\(escapeCSV(format))\",\"\(escapeCSV(file.reason))\",\"\(file.size)\",\"\(escapeCSV(fingerprint))\"\n"
        }
        ...
        if !repairs.isEmpty {
            ...
            for repair in repairs {
                let filePath = repair.fileURL?.path.replacingOccurrences(of: rootDirectory.path, with: "").trimmingCharacters(in: CharacterSet(charactersIn: "/")) ?? "Unknown"
                csv += "\"\(escapeCSV(filePath))\",\(repair.success),\(repair.fixed),\"\(escapeCSV(repair.message))\"\n"
            }
        }
