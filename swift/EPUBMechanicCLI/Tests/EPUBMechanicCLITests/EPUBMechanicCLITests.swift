...
struct EPUBMechanicCLITests {
    ...
    func runCLI(with arguments: [String]) throws -> String {
        let process = Process()
        process.executableURL = URL(fileURLWithPath: "./swift/EPUBMechanicCLI/.build/arm64-apple-macosx/debug/EPUBMechanicCLI")
    ...
    func testSpecCheck() throws {
        let output = try runCLI(with: ["validate", "--spec-check", "--dir", "swift/EbookMechanicCore/Tests/Resources/EPUBs"])
    ...
    func testFixMetadata() throws {
        ...
        let tempFilePath = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString).appendingPathExtension("epub")
        try FileManager.default.copyItem(at: testFilePath, to: tempFilePath)

        let output = try runCLI(with: ["repair", "--fix-metadata", "--dir", tempFilePath.deletingLastPathComponent().path])
    ...
    func testAccessibility() throws {
        let output = try runCLI(with: ["validate", "--spec-check", "--accessibility", "--dir", "swift/EbookMechanicCore/Tests/Resources/EPUBs"])
    ...
    func testExtractMetadata() throws {
        ...
        let output = try runCLI(with: ["validate", "--extract-metadata", "json", "--dir", testFilePath.deletingLastPathComponent().path])
