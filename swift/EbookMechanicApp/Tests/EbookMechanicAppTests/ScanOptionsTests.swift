...
final class ScanOptionsTests: XCTestCase {
  ...
  func testViewModelResetClearsState() {
    ...
    viewModel.corruptedFiles = [
      CorruptedFile(
        url: URL(fileURLWithPath: "/tmp/file.epub"), reason: "Invalid", size: 10, status: .corrupt)
    ]
  ...
  func testRunScanAutomationMovesCorruptedFilesAndDeletesEmptyFolders() async throws {
    ...
    XCTAssertTrue(
      viewModel.statusMessages.contains { $0.contains("Moved corrupted files to CORRUPTED") })
  ...
  func testRunScanWithNormalizationAndReportProducesArtifacts() async throws {
    ...
    XCTAssertEqual(viewModel.summary?.totalFiles, 1)
    XCTAssertEqual(
      viewModel.corruptedFiles.first?.reason,
      "Not a valid ZIP file (ZIP archive has invalid signature)")
    ...
    XCTAssertEqual(
      reportURL.deletingLastPathComponent().standardizedFileURL, tempDir.standardizedFileURL)
