                ...
                Section(header: Text("Corrupted Files (\(viewModel.corruptedFiles.count))")) {
                    if viewModel.corruptedFiles.isEmpty {
                        ...
                        ForEach(viewModel.corruptedFiles, id: \.url) { file in
                            Button {
                                selectedValidationResult = ValidationResult(originalIndex: 0, url: file.url, size: file.size, isValid: false, reason: file.reason, status: file.status, fingerprint: file.fingerprint, pdfValidationDetails: file.pdfValidationDetails, epubComplianceDetails: file.epubComplianceDetails)
                                showDetailView = true
                            } label: {
                                HStack {
...
extension Collection where Element == ValidationResult {
    func sortedByURL() -> [Element] {
        self.sorted { $0.url.lastPathComponent.localizedCaseInsensitiveCompare($1.url.lastPathComponent) == .orderedAscending }
    }
}
