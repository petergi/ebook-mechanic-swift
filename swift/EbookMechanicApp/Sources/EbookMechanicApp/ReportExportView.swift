                ...
                Section("Report Formats") {
                    ForEach(ReportFormat.allCases, id: \.self) { format in
                        Toggle(isOn: Binding(
                            ...
                            set: { if $0 { selectedFormats.insert(format) } else { selectedFormats.remove(format) } }
                        )) {
    ...
    private func exportReports() {
        ...
        Task {
            do {
                ...
                let urls = try await viewModel.generateReport(into: destinationFolder, options: options, formats: selectedFormats)
