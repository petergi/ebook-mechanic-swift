            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 20) {
                    if let metrics = viewModel.performanceMetrics {
                        Section("Overview") {
                            InfoRow(label: "Total Validation Time", value: String(format: "%.2f s", metrics.totalValidationTime))
                            InfoRow(label: "Files Per Second", value: String(format: "%.2f", metrics.filesPerSecond))
                            InfoRow(label: "Average Time Per File", value: String(format: "%.3f s", metrics.averageValidationTimePerFile))
                            InfoRow(label: "Cache Hit Rate", value: String(format: "%.1f %%", metrics.cacheHitRate * 100))
                        ...
                        if !metrics.validationTimeByFormat.isEmpty {
                            Section("Average Validation Time by Format") {
                                Chart(metrics.validationTimeByFormat.sorted(by: { $0.key.rawValue < $1.key.rawValue }), id: \.key) { format, time in
                                    BarMark(
                        ...
                        Section("Parallel Efficiency") {
                            ...
                            Text("A ratio closer to 1.0 indicates better utilization of parallel processing capabilities. A value above 1.0 may indicate overhead from parallelization for very fast tasks.")
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
                    } else {
                        Text("No performance metrics available. Run a scan with 'Show Performance Stats' enabled.")
                            .foregroundColor(.secondary)
                    }
                }
                .padding()
