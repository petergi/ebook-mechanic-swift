...
struct SettingsView: View {
    ...
    @AppStorage("defaultConcurrencyLevel") private var defaultConcurrencyLevel: Int = ProcessInfo.processInfo.activeProcessorCount
    @AppStorage("enableExternalToolsByDefault") private var enableExternalToolsByDefault: Bool = false
    @AppStorage("showPerformanceStatsByDefault") private var showPerformanceStatsByDefault: Bool = false
    ...
    var body: some View {
        Form {
            Section("General Settings") {
                ...
                Stepper("Default Concurrency Level: \(defaultConcurrencyLevel)", value: $defaultConcurrencyLevel, in: 1...16)
            }
            ...
            Section("Default Report Formats") {
                ForEach(ReportFormat.allCases, id: \.self) { format in
                    Toggle(format.rawValue.capitalized, isOn: Binding(
                        ...
                        set: { if $0 { selectedDefaultFormats.insert(format) } else { selectedDefaultFormats.remove(format) } }
                    ))
                }
            }
        }
    ...
    private func loadDefaultFormats() {
        if let decoded = try? JSONDecoder().decode(Set<ReportFormat>.self, from: defaultReportFormatsData) {
